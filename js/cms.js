...
(function () {
  ...
  function hydrateText(settings) {
    document.querySelectorAll("[data-cms]").forEach((node) => {
      ...
      // only touch the DOM when the value actually differs (avoids reflow/flicker)
      if (typeof val === "string" && val.length && node.textContent !== val) node.textContent = val;
    });
  ...
  // Build the fresh nodes into a detached clone, and only swap them into the
  // live container if the visible text actually changed. When the static HTML
  // already matches the database (the normal case), this is a no-op — so there
  // is no flicker or reflow on load.
  const norm = (s) => s.replace(/\s+/g, " ").trim();
  function swapIfChanged(box, buildInto) {
    if (!box) return false;
    const temp = document.createElement(box.tagName);
    buildInto(temp);
    if (norm(temp.textContent) === norm(box.textContent)) return false;
    box.replaceChildren(...temp.childNodes);
    return true;
  }

  function renderTestimonials(rows) {
    if (!rows.length) return;
    swapIfChanged(document.querySelector(".reviews"), (box) => {
      rows.forEach((r) => {
        ...
        fig.innerHTML =
          ...
          `<blockquote></blockquote><figcaption><strong></strong><span></span></figcaption>`;
  ...
  function renderPricing(rows) {
    if (!rows.length) return;
    swapIfChanged(document.querySelector(".pricing-grid"), (box) => {
      rows.forEach((t) => {
        const card = document.createElement("article");
        card.className = "price-card reveal in" + (t.featured ? " price-card--featured" : "");
        if (t.featured) {
          const flag = document.createElement("span");
          flag.className = "price-flag"; flag.textContent = "Most popular"; card.append(flag);
        }
        const h3 = document.createElement("h3"); h3.textContent = esc(t.name); card.append(h3);
        const amt = document.createElement("div"); amt.className = "price-amount"; amt.textContent = esc(t.price);
        ...
        a.className = "btn " + (t.featured ? "btn-primary" : "btn-outline"); a.href = "#audit";
  ...
  function renderFaqs(rows) {
    ...
    const changed = swapIfChanged(box, (target) => {
      rows.forEach((f) => {
  ...
  // Run hydration when the main thread is idle so it never competes with the
  // page's first paint or the user's first interactions.
  const runHydration = () =>
    Promise.allSettled([
  ...
  if ("requestIdleCallback" in window) requestIdleCallback(runHydration, { timeout: 2000 });
  else setTimeout(runHydration, 200);
})();
